...
class RetrieverConfig:
    ...
    category : str
        Категория: sparse, dense, hybrid, rerank.
    """
...
def apply_rerank(
    ...
    """
    Извлечение ``top_k * 3`` кандидатов через базовый компонент поиска,
    затем переранжирует cross-encoder моделью.

    Parameters
    ----------
    base : RetrieverConfig
        Базовый компонент поиска.
    reranker_model : str, optional
        Модель cross-encoder для переранжирования.

    Returns
    -------
    RetrieverConfig
        Ретривер с ре-ранкингом.
    """

    from llama_index.core.postprocessor import SentenceTransformerRerank

    reranker = SentenceTransformerRerank(model=reranker_model, top_n=5)
    ...
    def _retrieve(query: str, top_k: int) -> list[NodeWithScore]:
        ...
        candidates = base.retrieve(query=query, top_k=top_k * 3)
        reranker.top_n = top_k

        return reranker.postprocess_nodes(candidates, QueryBundle(query_str=query))

    return RetrieverConfig(
        name=f"{base.name}+rerank",
        description=f"{base.description} + cross-encoder rerank",
        category="rerank",
        _retrieve_fn=_retrieve,
    )
